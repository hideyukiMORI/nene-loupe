...
ウィンドウ手続きは操作を意図として渡し、描画はapplicationの表示値を反映する（ARC-011）。
タイマーは再サンプリングのきっかけで、値を累積しない。メッセージ番号は開いたOSの集合なので、
`DefWindowProcW`へ渡す既定分岐を許す。閉じた業務enumの既定分岐とは区別する（CPP-002）。
HWND・HDC・HBITMAP等の所有権はRAIIで閉じる。実機証明はquality/gate-proofs.mdに記録する。
描画はメモリDCで完成させてから転送し、更新途中の画面を見せない（Issue #3）。
OSコールバックはnoexceptとし、想定外の例外をOSフレームへ流さない。資源枯渇による
std::bad_alloc等は欠陥・想定外として終了し、捕捉して成功扱いにはしない。
